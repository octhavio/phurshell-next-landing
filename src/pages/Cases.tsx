...
export default function Cases() {
  ...
  const categories = [
    ...
    { id: 'mobile', name: 'Mobile', count: cases.filter(c => c.tags.some(t => t.includes('IOS') || t.includes('ANDROID'))).length },
    { id: 'web', name: 'Web', count: cases.filter(c => c.tags.some(t => t.includes('WEB'))).length },
    { id: 'backend', name: 'Backend', count: cases.filter(c => c.tags.some(t => t.includes('BACKEND'))).length },
  ]
        ...
        <div className="container mx-auto max-w-screen-2xl px-10 sm:px-14 lg:px-20">
          ...
          <div className="space-y-8">
            {cases.map((caseItem, index) => (
              <div
                key={index}
                className="border border-dark/10 bg-white overflow-hidden rounded-button"
              >
                  ...
                  <div className="space-y-8">
                    ...
                    <div className="flex flex-wrap gap-2">
                      {caseItem.tags.map((tag, tagIndex) => (
                        <span
                          key={tagIndex}
                          className="rounded-full border border-dark/10 bg-white px-4 py-2 text-sm font-bold text-dark/60"
                        >
                          {tag}
                        </span>
                      ))}
